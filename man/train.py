...
print('max length: ', max([len(x) for x in subdata.X_split.values]))
maxlen = 105
# tokenizer: char. level
X = tokenizer.texts_to_sequences(subdata.X_split.values)
# padding
X = sequence.pad_sequences(X, maxlen=maxlen, truncating='post', padding='pre')
# one-hot x_add(equip)
...
h = Embedding(len(tokenizer.index_word) + 1, 64,
              input_length=maxlen,
              trainable=True)(x)
h = GRU(128, return_sequences=True, unroll=True,
        dropout=0.25, recurrent_dropout=0.25)(h)
h = Attention(maxlen)(h)
...
# train
hist = model.fit(X, Y, epochs=250, batch_size=128,
                 shuffle=True, validation_split=0.2,
                 verbose=2)
np.save('4class_250epoch_text_only.npy', hist.history)
